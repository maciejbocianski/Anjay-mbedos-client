...
static int fw_stream_write(void *user_ptr, const void *data, size_t length) {
    (void) user_ptr;
    total_size += length;
    printf("[mboc] fw_stream_write length(%lu) total_size(%lu)\n", length, total_size);
#if 0
...
static int fw_stream_finish(void *user_ptr) {
    (void) user_ptr;
    printf("[mboc] fw_stream_finish total_size(%lu)\n", total_size);
    total_size = 0;
#if 0
    assert(FW_STATE.firmware_file != NULL);
...
static int fw_perform_upgrade(void *user_ptr) {
    printf("[mboc] fw_perform_upgrade\n");
#if 0
    ...
    (void) execl(FW_IMAGE_DOWNLOAD_NAME, FW_IMAGE_DOWNLOAD_NAME, DEFAULT_ENDPOINT_NAME,
                 NULL);
...
#endif
    printf("[mboc] fw_perform_upgrade - done\n");
    return -1;
}
